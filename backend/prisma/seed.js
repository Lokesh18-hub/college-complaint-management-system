...
async function main() {
    ...
    const students = await Promise.all(studentsData.map((s) => prisma.user.create({ data: s })));
    ...
    const staffMembers = await Promise.all(staffData.map((s) => prisma.staff.create({ data: s })));
